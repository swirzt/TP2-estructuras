...
void swap_char(void* origen, Cola cola, PalabraMal palabraActual){
  ...
  wchar_t temp;
  for (int i = 0; i<largo-1 && palabraActual->sizesugerencias<5; i++){
    temp = palabra[i];
    palabra[i] = palabra[i+1];
    palabra[i+1] = temp;
    if (tablahash_buscar(tabla, palabra, largo)){
       wcscpy(palabraActual->sugerencias[palabraActual->sizesugerencias], palabra);
...
void replace_char(void* origen, Cola cola, PalabraMal palabraActual){
  wchar_t* palabra = origen;
  int largo = wcslen(palabra);
  for (int j = 0; j < largo && palabraActual->sizesugerencias<5; j++){
    wchar_t viejochar = palabra[j];
    for (int i = 'a'; i <= 'z' && palabraActual->sizesugerencias<5; i++) {
      palabra[j] = i;
      if (tablahash_buscar(tabla, palabra, largo)){
         wcscpy(palabraActual->sugerencias[palabraActual->sizesugerencias], palabra);
         palabraActual->sizesugerencias++;
...
void remove_char(void* origen, Cola cola, PalabraMal palabraActual){
  ...
  for (int j = 0; j < largo && palabraActual->sizesugerencias<5; j++){
    wchar_t viejochar = palabra[j];
    for(int i = j; i < largo && palabraActual->sizesugerencias<5 ; i++){
      palabra[i] = palabra [i+1];
    }
    if (tablahash_buscar(tabla, palabra, largo)){
       wcscpy(palabraActual->sugerencias[palabraActual->sizesugerencias], palabra);
...
void insert_char(void* origen, Cola cola, PalabraMal palabraActual){
    ...
    for (int j = 0; j <= largo && palabraActual->sizesugerencias<5; j++){
      ...
      for(int k='a'; k<='z' && palabraActual->sizesugerencias<5;k++){
        buffer[j] = k;
      ...
      if(j>0 || j < largo && palabraActual->sizesugerencias<5) buffer[j] = ' ';
      cola_encolar(cola, palabra);
    }
}
...
void crea_correciones(Cola palabras, TablaHash* dicc) {
  ...
  while(palabras != NULL){
    ...
    cola_desencolar(palabras);
    cola_encolar(colaPalabras, palabraActual->dato);
    while(palabraActual->sizesugerencias < 5 && salida){
      replace_char(cola_primero(colaPalabras), colaPalabras, palabraActual);
      remove_char(cola_primero(colaPalabras), colaPalabras, palabraActual);
      swap_char(cola_primero(colaPalabras), colaPalabras, palabraActual);

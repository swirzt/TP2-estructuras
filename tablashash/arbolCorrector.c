...
int btree_obtener_dato(BTree arbol, void* string){
  ...
  while(!encontre && arbol!=NULL){
    if(!strcmp(arbol->dato, palabra)) encontre = 1;
    else if(strcmp(arbol->dato, palabra) < 0) arbol = arbol->right;
    else arbol = arbol->left;
    }
...
BTree btree_insertar(BTree arbol, void* string){
  ...
  while (arbol != NULL) {
    padre = arbol;
    if(strcmp(arbol->dato, palabra) < 0) arbol = arbol->right;
    else if(strcmp(arbol->dato, palabra) > 0)  arbol = arbol->left;
  }
  BTree nuevoNodo = malloc(sizeof(BTNodo));
  nuevoNodo->dato = malloc(sizeof(char)*256);
  nuevoNodo->dato = strcpy(nuevoNodo->dato, palabra);
  nuevoNodo->right = NULL;
  nuevoNodo->left = NULL;
  if(strcmp(padre->dato, palabra) < 0) padre->right = nuevoNodo;
  else padre->left = nuevoNodo;
  return origen;
}
